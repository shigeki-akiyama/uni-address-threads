...
namespace comm {
        ...
        , poll_arg_()
    {
        ...
        // This does not work in ibv-conduit???
        ...
        int n_amentries = sizeof(amentries) / sizeof(*amentries);

        uintptr_t segsize = gasnet_getMaxGlobalSegmentSize();
        uintptr_t minheapoffset = 0;

        // FIXME: set with environment variable
        if (segsize >= 4 * 1024 * 1024)
            segsize = 4 * 1024 * 1024;
