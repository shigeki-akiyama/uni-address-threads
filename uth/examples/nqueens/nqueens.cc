...
static long
spawn_rec_tasks(int begin, int end, int n, int j, board b, int depth)
{
    if (end - begin == 1) {
        int i = begin;
        ...
        madm::future<long> f(spawn_rec_tasks,
                             begin, center, n, j, b, depth);

        long solutions1 = spawn_rec_tasks(center, end, n, j, b, depth);

        long solutions0 = f.touch();
...
static long nqueens(int n, int j, board b, int depth)
{
    ...
    if (n == j) {
        ...
        if (g_n_cutoff != 0 && depth >= g_n_cutoff) {
            ...
            #if BINARY_TASKS
                solutions = spawn_rec_tasks(0, n, n, j, b, depth);
            #else
...
void real_main(int argc, char **argv)
{
    ...
    long total_count = 0;
    if (me == 0) {
        madm::future<long> f(nqueens_root, n);
        total_count = f.touch();

//        total_count = nqueens_ser(n, 0, a);
    }
